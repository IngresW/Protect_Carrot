...
BulletBase* BulletBase::create(const int& rIId, EntityAffected* rEntityAffected)
{
    auto pBullet = new BulletBase();
    if (pBullet && pBullet->init(rIId, rEntityAffected)) pBullet->autorelease();
    else CC_SAFE_DELETE(pBullet);
    return pBullet;
}

bool BulletBase::init(const int& rId, EntityAffected* rEntityAffected)
{
    bool bRet = false;
    ...
    do
    {
        ...
        CC_BREAK_IF(!DynamicInterface::init(rId, BULLETCSVFILE));
        ...
        CC_SAFE_RETAIN(rEntityAffected);
        _pAtkTarget = rEntityAffected;
        _IdMap = new std::set<int>();

        BulletCreate::getInstance()->addBullet(this);
...
void BulletBase::setAction(const float& rDt)
{
    switch (_iBulletType)
    {
    ...
    case en_GunBullet:
    {
        ...
        _Animation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(_sModelName + "1" + PHOTOPOSTFIX));
    ...
    case en_NoDisspper:
    {
        getSprite()->runAction(RepeatForever::create(RotateBy::create(0.7f, 360)));
        break;
    }
    default:
        break;
